// from https://code.google.com/p/waka-go/source/browse/
...
func main_ftp() {
	laddr, err := net.ResolveTCPAddr("tcp4","localhost:21")
	if err != nil {
		log.Fatal("ftp-server-e1: ",err)
	}
	listener, err := net.ListenTCP("tcp4", laddr)
	if err != nil {
		log.Fatal("ftp-server-e2: ",err)
	}
	ftpServer := &FTPServer {
		"Go FTP Server",
		listener,
	...
	log.Fatal("ftp-server-e3: ",ftpServer.Listen())
}